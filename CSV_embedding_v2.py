...
def generate_mapped_dataframe(source_df, mapping_data):
    """Generate a new DataFrame based on the mapping"""
    # Create an empty DataFrame for the mapped data
    mapped_df = pd.DataFrame()
    
    # Track which source columns are mapped to which destination columns
    mapping_dict = {}
    original_columns = []
    mapped_dest_columns = set()  # Track which destination columns are mapped
    
    for row in mapping_data:
        source_col = row["Column of Uploaded CSV"]
        dest_col = row["Mapped Field of Data Standard"]
        
        if dest_col != "Unmapped":
            if dest_col == "Original Column Name":
                # Keep the original column name
                original_columns.append(source_col)
            else:
                # Map to a destination column
                mapping_dict[source_col] = dest_col
                mapped_dest_columns.add(dest_col)
    
    # Add mapped columns
    for source_col, dest_col in mapping_dict.items():
        try:
            # Get the source data
            source_data = source_df[source_col]
            
            # Find the corresponding mapping row
            mapping_row = next(row for row in mapping_data if row["Column of Uploaded CSV"] == source_col)
            
            # Check if types match
            if mapping_row["Type Match"]:
                # Types match, copy directly
                mapped_df[dest_col] = source_data
            else:
                # Types don't match, try to convert
                try:
                    # Get destination type
                    dest_type = mapping_row["Destination Type"]
                    
                    if dest_type == "numeric":
                        # Try to convert to numeric
                        mapped_df[dest_col] = pd.to_numeric(source_data, errors='coerce')
                    elif dest_type == "datetime":
                        # Try to convert to datetime
                        mapped_df[dest_col] = pd.to_datetime(source_data, errors='coerce')
                    else:
                        # Convert to string
                        mapped_df[dest_col] = source_data.astype(str)
                except:
                    # If conversion fails, keep original data
                    mapped_df[dest_col] = source_data
                    st.warning(f"Could not convert column '{source_col}' to {dest_type}. Keeping original data type.")
        except Exception as e:
            st.error(f"Error mapping column '{source_col}' to '{dest_col}': {str(e)}")
    
    # Add original columns that were marked to keep
    for col in original_columns:
        if col in source_df.columns:
            mapped_df[col] = source_df[col]
    
    # Check for missing destination columns and add them with "Data Missing" values
    if st.session_state.current_destination_id:
        dest_df = st.session_state.destination_files_dict[st.session_state.current_destination_id]["data"]
        data_standard = st.session_state.destination_files_dict[st.session_state.current_destination_id]["standard"]
        
        # Find destination columns that weren't mapped
        for dest_col in dest_df.columns:
            if dest_col not in mapped_dest_columns:
                # Add this column with "Data Missing" values
                mapped_df[dest_col] = "Data Missing"
                st.info(f"Added missing destination column '{dest_col}' with 'Data Missing' values")
...
def main():
    ...
    if st.session_state.current_source_id and st.session_state.current_destination_id:
        ...
        if st.session_state.get('show_analysis', False):
            ...
            # Display column information in aligned expanders
            st.markdown("### Column Information")
            col1, col2 = st.columns(2)

            with col1:
                with st.expander("Source Columns", expanded=True):
                    st.dataframe(source_col_info, use_container_width=True)

            with col2:
                with st.expander("Destination Columns", expanded=True):
                    st.dataframe(dest_col_info, use_container_width=True)
            ...
            if st.button("Start Column Mapping") or st.session_state.mapping_data is not None:
                ...
                if st.session_state.mapping_data is None:
                    ...
                    for col in df.columns:
                        ...
                        mapping_data.append({
                            ...
                            "Keep Original": False  # Add this field to track if we should keep the original column
                        })
                ...
                with tab1:
                    ...
                    for idx, row in mapping_df.iterrows():
                        col1, col2, col3, col4, col5, col6, col7 = st.columns([2, 1.5, 2, 1, 1, 1, 1])
                        ...
                        with col3:
                            if st.session_state.editing_row == idx:
                                # Add "Original Column Name" to the options
                                standard_options = list(data_standard.keys()) + ["Original Column Name", "Unmapped"]
                                
                                # Determine the current index
                                if row["Mapped Field of Data Standard"] == "Original Column Name":
                                    current_index = len(standard_options) - 2  # Second to last option
                                elif row["Mapped Field of Data Standard"] in standard_options:
                                    current_index = standard_options.index(row["Mapped Field of Data Standard"])
                                else:
                                    current_index = len(standard_options) - 1  # Last option (Unmapped)
                                ...
                                if new_value != "Unmapped" and new_value != "Original Column Name":
                                    ...
                                    st.session_state.mapping_data[idx]["Keep Original"] = False
                                elif new_value == "Original Column Name":
                                    st.session_state.mapping_data[idx]["Destination Type"] = row["Source Type"]
                                    st.session_state.mapping_data[idx]["Type Match"] = True
                                    st.session_state.mapping_data[idx]["Keep Original"] = True
                                else:
                                ...
                                if row["Mapped Field of Data Standard"] == "Original Column Name":
                                    st.write(f"Original Column Name ({row['Source Type']})")
                                else:
